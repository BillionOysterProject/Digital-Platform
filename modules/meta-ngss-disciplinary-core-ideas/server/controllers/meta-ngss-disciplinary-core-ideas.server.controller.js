...
exports.create = function (req, res) {
  var standard = new MetaNgssDisciplinaryCoreIdea(req.body);
...
exports.list = function (req, res) {
  ...
  if (req.query.searchString) {
    try {
      ...
      query = MetaNgssDisciplinaryCoreIdea.find({ $or: [{ 'header': searchRe }, { 'description': searchRe }] });
    } catch(e) {
      return res.status(400).send({
    ...
    query = MetaNgssDisciplinaryCoreIdea.find();
  }
...
exports.standardByID = function (req, res, next, id) {
  ...
  MetaNgssDisciplinaryCoreIdea.findById(id).exec(function (err, standard) {
    if (err) {
      return next(err);
    } else if (!standard) {
