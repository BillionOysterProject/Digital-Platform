...
exports.create = function (req, res) {
  var standard = new MetaCclsMathematics(req.body);
...
exports.list = function (req, res) {
  MetaCclsMathematics.find().sort('order').exec(function (err, standards) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.standardByID = function (req, res, next, id) {
  ...
  MetaCclsMathematics.findById(id).exec(function (err, standard) {
    if (err) {
      return next(err);
    } else if (!standard) {
