...
exports.create = function (req, res) {
  var standard = new MetaCclsElaScienceTechnicalSubject(req.body);
...
exports.list = function (req, res) {
  MetaCclsElaScienceTechnicalSubject.find().sort('order').exec(function (err, standards) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.standardByID = function (req, res, next, id) {
  ...
  MetaCclsElaScienceTechnicalSubject.findById(id).exec(function (err, standard) {
    if (err) {
      return next(err);
    } else if (!standard) {
