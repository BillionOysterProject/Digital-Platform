...
exports.create = function (req, res) {
  var standard = new MetaNgssCrossCuttingConcept(req.body);
...
exports.list = function (req, res) {
  ...
  if (req.query.searchString) {
    try {
      ...
      query = MetaNgssCrossCuttingConcept.find({ $or: [{ 'header': searchRe },{ 'description': searchRe }] });
    } catch(e) {
      return res.status(400).send({
    ...
    query = MetaNgssCrossCuttingConcept.find();
  }
...
exports.standardByID = function (req, res, next, id) {
  ...
  MetaNgssCrossCuttingConcept.findById(id).exec(function (err, standard) {
    if (err) {
      return next(err);
    } else if (!standard) {
